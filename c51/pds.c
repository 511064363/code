...
void PwmInit(void)
{
    ...
    PWMA_CCMR4 = 0x68;           //设置CC1为PWMA输出模式
    PWMA_CCER2 = 0xF0;           //使能CC1通道
	PWMA_ENO = 0x00;			 //0000 0000
	PWMA_PS = 0x00;
	PWMA_BKR = 0x80;             //使能主输出
    PWMA_CR1 = 0x01;             //开始计时
}
...
void PwmSet(u8 i)
{
	PWMA_CCR4H = (u8)(PwmCcr1>>8);      //0x0100设置占空比时间
    PWMA_CCR4L = (u8)(PwmCcr1);
...
{
	u8 j;
	u16 k[14],imin,imax,isum;
	for(j=0;j<14;j++) k[j]=adcV;
...
void ICC()
{
	...
	switch (ch)
	{
		case 0x00:	//V=2500/4096*10
			i=adcV*6.1035;
			break;
		case 0x01:	//i=2500/4096/0.01/50*10
			i=adcV*12.207;
			if((i/10)>(ic*10))
			{
				PT=0;
...
//					}
//				}
//				else if(recv_buf[2]==0x01)	//PwmCcr1
//				{
...
//			}	
		...
		if(rNG)	
		{
			...
			if(recv_buf[3]==0x02)
			{
				if(recv_buf[2]==0x00)	//周期
					PwmArr = (recv_buf[4]<<8)|recv_buf[5];
				else if(recv_buf[2]==0x01)	//占空比
					PwmCcr1 = (recv_buf[4]<<8)|recv_buf[5];
			}
		}
		Delay50ms();
