...
void main(void)
{
	u8 i=0;u8 *ID;ID=(char idata *)0xf1;
	IO_Init();
	Timer4Init();
	UartInit();
			...
			if(rOK){
				...
				if(recv_buf[3]==0x01){
					switch(recv_buf[2])
						...
						case 0xee:
							for (i=0; i<7; i++)	UartSend(ID[i]);	//打印芯片全球唯一ID
						break;
						
						default:
							if(recv_buf[2]) Ln=recv_buf[2]*10;		//改变闪灯频率
						break;
						}
			}}
...
void LVD_Isr() interrupt 6
{
    ...
    P3M0 = 0x00; P3M1 = 0xff; 
    P4M0 = 0x00; P4M1 = 0xff; 
	  IapProgram(adr,Ln);IapProgram(adr+1,Lm);
	  Delay100ms();
}