...
for j in listDir:
    if os.path.splitext(j)[1] == ".txt":
        ...
        f2(j, '#')  # 处理异常帧
        try:
            (a, b, c, d, e) = np.loadtxt(j, dtype=int, skiprows=1, delimiter=',', comments='#', usecols=(0, 1, 2, 3, 4),
                                         converters=None, unpack=True)
        except Exception as e:
            # print(win32api.MessageBox(0,str(e),"警告", win32con.MB_ICONWARNING))
            sys.exit()

        ti = str(z) + '_' + os.path.splitext(j)[0] + '_ADC' + str(int(np.mean(b))) + '_KEY' + str(
            np.sum(np.diff(a) == 16)) + '_' + __time
        ...
        plt.title(ti, fontsize=30,color='blue')
        plt.plot(b * bx - by + np.ptp(c) + 30, color='green', label='adc', alpha=apa, linewidth=0.8)
