...
for j in listDir:
    if os.path.splitext(j)[1] == ".txt":
        ...
        plt.title(os.path.splitext(j)[0]+'_'+__time,fontsize=50)
        plt.plot(b, color='green', label='adc', alpha=1, linewidth=0.8)
        plt.plot(c, color='blue', label='adm', alpha=1, linewidth=0.8)
        plt.plot(d, color='orange', label='base', alpha=1, linewidth=0.8)
        plt.plot(e, color='red', label='key', alpha=1, linewidth=0.8)
